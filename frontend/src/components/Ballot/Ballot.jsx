...
const Ballot = ()=>{
    ...
    return (
        <div className="h-full w-full bg-blue-500 flex  justify-center ">
            <div className="m-24 w-11/12 h-3/4 px-10 py-16 flex flex-col items-center justify-center bg-white rounded-lg gap-4 ">
                ...
                <ul className="w-full h-full gap-1 overflow-auto">
                    {
                        candidates.map((candidate)=>{
                            return <li key={candidate.id} className="p-4 m-2 rounded-lg  bg-cyan-50 flex justify-between items-center ">{candidate.name} <button className="bg-green-200 px-4 py-2 rounded-lg ">Vote</button> </li>
                        })
                    }
                </ul>
            </div>
